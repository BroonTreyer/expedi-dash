...
const navTree: NavNode[] = [
  ...
  // 2. Expedição (operação de saída)
  {
    ...
    children: [
      ...
      { to: "/distribuidores", label: "Distribuidores", icon: Building2, roles: ["admin", "logistica", "faturamento"] },
...
export function AppSidebar({ collapsed, onNavigate }: Props) {
  ...
  return (
    <TooltipProvider delayDuration={0}>
      <aside className={cn(
        "h-dvh bg-sidebar text-sidebar-foreground flex flex-col border-r border-sidebar-border transition-all duration-200 overflow-y-auto pt-[env(safe-area-inset-top)] pb-[env(safe-area-inset-bottom)]",
        collapsed ? "w-14" : "w-60"
      )}>
