...
export function CarregamentoTable({ data, onStatusChange, onEdit, onDelete }: Props) {
  return (
        ...
        <TableBody>
          ...
          {data.map((c) => (
            <TableRow key={c.id} className="hover:bg-muted/30">
              ...
              <TableCell className="text-sm">{c.tipo_caminhao ?? "—"}</TableCell>
              <TableCell className="text-sm font-mono uppercase">{c.placa ?? "—"}</TableCell>
