...
export const KpiCards = React.memo(function KpiCards({ data }: Props) {
  const totalClientes = new Set(data.filter(c => c.codigo_cliente).map(c => c.codigo_cliente)).size;
  ...
  const cards = [
    { label: "Clientes", value: totalClientes, icon: Package, color: "text-primary" },
