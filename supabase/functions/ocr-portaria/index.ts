...
async function ocrLacreGemini(imageUrl: string): Promise<{ texto: string; confianca: number }> {
  const LOVABLE_API_KEY = Deno.env.get("LOVABLE_API_KEY");
  if (!LOVABLE_API_KEY) throw new Error("LOVABLE_API_KEY not configured");
  ...
  const response = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: "system",
          content: "Você é um sistema de OCR especializado em leitura de lacres de segurança. Analise a imagem e extraia o número do lacre mostrado. Use a tool read_result para retornar o resultado.",
        },
        {
          role: "user",
          content: [
            { type: "text", text: "Leia o número do lacre de segurança nesta imagem. Retorne o número (apenas números e letras) e um nível de confiança de 0 a 100." },
            { type: "image_url", image_url: { url: imageUrl } },
          ],
        },
      ],
      tools: [
        {
          ...
          function: {
            name: "read_result",
            description: "Return the OCR reading result for the seal number",
            parameters: {
              type: "object",
              properties: {
                texto: { type: "string", description: "Número do lacre lido (ex: 123456)" },
                confianca: { type: "number", description: "Nível de confiança da leitura de 0 a 100" },
              },
              required: ["texto", "confianca"],
              additionalProperties: false,
  ...
  if (!response.ok) {
    const errText = await response.text();
    console.error("AI gateway error (lacre):", response.status, errText);
    throw new Error(`AI error: ${response.status}`);
  }
