...
BEGIN
  IF NEW.etapa = 'logistica'
    ...
    -- Vincula apenas movimentações de entrada AINDA EM ANDAMENTO (não finalizadas)
    -- e recentes (24h), para não herdar chegadas antigas/encerradas do mesmo veículo.
    ...
    WHERE upper(trim(placa)) = upper(trim(NEW.placa))
      ...
      AND COALESCE(etapa_carga_propria, '') <> 'finalizado'
