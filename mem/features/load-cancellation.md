...
2. Confirmation: insert into `ocorrencias_carga` (motivo, observacao, full carga snapshot, registrado_por), reverts all `carregamentos_dia` rows of that `carga_id` to `etapa='vendas'` and clears carga_id/nome_carga/placa/motorista/transportadora/tipo_caminhao/horario_inicio/horario_fim/ordem_entrega, deletes `veiculos_esperados` of that carga_id, deletes pending arrival movement (only if `horario_entrada is null`), logs `audit_log` action='cancelada'.
